    ...
    public Response<Object> regUser(@PathVariable("userid") String userid, @RequestBody String body) {
        ...
        switch (algorithm) {
            case "AES":
                ...
                txt = CryptUtil.encryptAesBase64(keyLen16, body);
                break;
            case "SM4":
                map.put(AlgEnmu.ALGORITHM_KEY.algorithm(), redisKeyService.getAlgorithmKey(atoken));
                txt = CryptUtil.encryptSm4Base64(keyLen16, body);
                break;
            case "SM2":
                /* 服务端密钥对 */
                var serverSm2Map = CryptUtil.generateSm2Key();
                /* 客户端密钥对 */
                var clientSm2Map = CryptUtil.generateSm2Key();
                Map<String, String> clientKeyMap = HashMap.newHashMap(VarEnmu.SIXTEEN.ivalue());
                var priksm4 = CryptUtil.encryptSm4Base64(keyLen16, clientSm2Map.get(VarEnmu.PRIVATE_KEY.value()));
